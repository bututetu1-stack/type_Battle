...
interface CustomCfg {
  ...
  soloMode: 'royale' | 'timeattack'; // ソロのモード（バトル / タイムアタック）
  taSeconds: number; // タイムアタックの制限時間（秒）
}
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        soloMode: o.soloMode === 'timeattack' ? 'timeattack' : 'royale',
        taSeconds: typeof o.taSeconds === 'number' ? o.taSeconds : def.taSeconds,
      };
    }
...
export default function SoloGame({ onExit }: { onExit: () => void }) {
  ...
  const savedTsRef = useRef<number | null>(null); // 自動保存した記録の識別子（改名で在place更新）
  ...
  // ソロのモード（バトル / タイムアタック）と制限時間。
  const [soloMode, setSoloMode] = useState<'royale' | 'timeattack'>(initialCfg.soloMode);
  const [cfgTaSeconds, setCfgTaSeconds] = useState(initialCfg.taSeconds);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CFG_KEY, JSON.stringify({
        ...
        soloMode, taSeconds: cfgTaSeconds,
      }));
    } catch { /* 保存不可環境は無視 */ }
  }, [cfgInitial, cfgMin, cfgAccel, theme, cfgHp, cfgEnemies, cfgCpuStr, cfgTreasureRate, cfgAttackGauge, cfgAttackCap, cfgComboStep, cfgBadgeCap, cfgBadgeRate, cfgGaugeMode, cfgGaugeChars, cfgComeback, cfgAutoFull, cfgUse, cfgItemsOn, cfgDisabledItems, cfgExcluded, soloMode, cfgTaSeconds]);
  ...
  const dazzleUntilRef = useRef(0); // 視認性低下（CPUから受けてお題パネルがゲーミング化）の終了時刻
  ...
  const startGame = useCallback(() => {
    ...
    if (soloMode === 'timeattack') {
      // タイムアタック: 敵なし・被攻撃なし。制限時間だけで終了。
      setDummies([]);
      setTimeLeft(cfgTaSeconds);
    } else {
    ...
    sfx.start();
  }, [cfgInitial, cfgMin, cfgAccel, cfgHp, cfgEnemies, cfgCpuStr, cfgAttackGauge, cfgExcluded, soloMode, cfgTaSeconds, updatePending, nextWord]);
  ...
  useEffect(() => {
    if (gameState !== 'playing' || soloMode === 'timeattack') return; // タイムアタックは敵なし
    const interval = setInterval(() => {
      ...
      if (itemsOnRef.current && Math.random() < 0.15) {
        // プレイヤーが無効化したアイテムはCPUも使わない（grantItem と同じゲート）。
        const cpuPool = ALL_ITEMS.filter((it) => !disabledItemsRef.current.has(it));
        if (cpuPool.length > 0) {
        const user = alive[Math.floor(Math.random() * alive.length)];
        const item = cpuPool[Math.floor(Math.random() * cpuPool.length)];
        ...
        if (item === 'longbomb' && !alreadyHasLong && incomingNew < 6 && Date.now() >= freezeUntilRef.current) {
          ...
          // 視認性低下: プレイヤーのお題パネルを一定時間ゲーミングに歪ませる。
          dazzleUntilRef.current = Date.now() + DAZZLE_DURATION;
          setNowTick(Date.now());
          pushToast(`${user.name} から視認性低下 🌈`, 'in');
        } else {
  ...
  }, [gameState, soloMode, fireIncoming, pushToast, updatePending, addBeam]);
      ...
      if (now < brakeUntilRef.current || now < freezeUntilRef.current) {
        ...
        setBacklog((prev) => {
          if (prev.length >= maxBacklogRef.current) {
            // タイムアタックは敗北なし。満杯なら供給を止めるだけ（時間切れで終了）。
            if (soloModeRef.current === 'timeattack') return prev;
  ...
  // 結果を端末ローカルのハイスコア表に自動保存（1試合1回）。タイムアタックはグローバルにも送信。
  ...
  // 結果画面に入ったら記録を自動保存（名前は端末に記憶した表示名を使用）。
  useEffect(() => {
    if ((gameState === 'gameover' || gameState === 'win' || gameState === 'timeup') && !hasSaved && startTime) {
      handleSaveRecord();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [gameState]);

  // 自動保存後の表示名の変更: 端末の記録名（在place）と記憶名を更新。
  const handleRenameChange = (v: string) => {
    setPlayerName(v);
    const nm = v.trim() || 'プレイヤー';
    savePlayerName(nm);
    if (savedTsRef.current != null) renameScore(savedTsRef.current, nm);
  };
  // 入力確定時にオンライン（タイムアタック）の表示名も最新へ。
  const handleRenameBlur = () => {
    if (soloMode !== 'timeattack' || !hasSaved) return;
    const nm = playerName.trim() || 'プレイヤー';
    submitGlobalScore({
      name: nm,
      score: currentScore(),
      keys: keysTyped,
      ...
      .then((res) => setGlobalRank(res.rank))
      .catch(() => {});
  };
  ...
  return (
    <div className={`h-screen bg-transparent text-text font-sans overflow-hidden flex flex-col selection:bg-cyan-900 ${shake && keyCfg.screenShake ? 'screen-shake' : ''}`}>
            ...
            <div className="fixed left-0 right-0 top-16 bottom-0 bg-neutral-950/90 backdrop-blur-sm z-30 overflow-y-auto">
              ...
              <div className="mb-4 w-full max-w-sm">
                <div className="grid grid-cols-2 gap-2">
                  {([['royale', '⚔ バトル', '敵CPUと対戦'], ['timeattack', '⏱ タイムアタック', '制限時間でスコア']] as const).map(([m, lbl, desc]) => (
                    <button key={m} onClick={() => setSoloMode(m)}
                      className={`rounded-lg px-3 py-2 text-left border transition-colors ${soloMode === m ? 'bg-cyan-600/20 border-cyan-500 text-cyan-200' : 'bg-neutral-800 border-white/10 text-gray-400 hover:bg-neutral-700'}`}>
                      <div className="text-sm font-bold">{lbl}</div>
                ...
                {soloMode === 'timeattack' && (
                  <div className="mt-2 flex items-center justify-center gap-2 text-[11px] text-gray-400">
                    <span>制限時間</span>
                    {[30, 60, 90, 120].map((s) => (
                      <button key={s} onClick={() => setCfgTaSeconds(s)}
                        className={`px-2 py-0.5 rounded text-[10px] font-bold transition-colors ${cfgTaSeconds === s ? 'bg-cyan-600 text-white' : 'bg-neutral-800 text-gray-400 hover:bg-neutral-700'}`}>
                        {s}秒
                      </button>
                    ))}
                  </div>
              ...
              <div className="mb-6 w-full max-w-sm">
                ...
                <div className="mt-3">
                  ...
                  <div className="flex flex-wrap gap-1 justify-center">
                    {/* ビルトインテーマ ＋ 自作テーマ（custom:名）を同じ感覚でON/OFF */}
                    {[
                      ...THEMES.filter((t) => t.id !== 'all' && t.id !== 'custom').map((t) => ({ id: t.id, label: t.label })),
                      ...customGroups.map((g) => ({ id: `custom:${g}`, label: `🗂${g}` })),
                    ].map((t) => {
      ...
      <style
        ...
        /* 視認性低下（CPUから受ける）: お題パネル自体を虹色シフト＋ブレで歪ませる。 */
